    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
      ...
      <main className="mx-auto max-w-5xl space-y-6 p-6 lg:p-8">
        ...
        <PermissionGuard permission={WORKSPACE_PERMISSION.WORKSPACE_UPDATE}>
          <WorkspaceGeneralSettings workspace={currentWorkspace} />
        </PermissionGuard>

        {/* Branding */}
        <WorkspaceBrandingSettings workspace={currentWorkspace} />

        {/* Members */}
        <PermissionGuard permission={WORKSPACE_PERMISSION.MEMBER_READ}>
          <WorkspaceMemberSettings workspace={currentWorkspace} />
        </PermissionGuard>

        {/* Invitations */}
        <PermissionGuard permission={WORKSPACE_PERMISSION.INVITATION_READ}>
          <WorkspaceInvitationSettings workspace={currentWorkspace} />
        </PermissionGuard>

        {/* Security */}
        <WorkspaceSecuritySettings workspace={currentWorkspace} />

        {/* Owner only */}
        <PermissionGuard permission={WORKSPACE_PERMISSION.WORKSPACE_DELETE}>
          <WorkspaceDangerZone workspace={currentWorkspace} />
        </PermissionGuard>
      </main>
    </div>
  );
