...
describe('OAuthService', () => {
  ...
  describe('loginWithGoogle', () => {
    ...
    it('should login successfully with existing oauth account', async () => {
      ...
      const result = await oauthService.loginWithGoogle('google-code', {
        ipAddress: '127.0.0.1',
        userAgent: 'Chrome',
        rememberMe: true,
      });

      expect(googleProvider.exchangeCode).toHaveBeenCalledWith('google-code');

      expect(googleProvider.getUserInfo).toHaveBeenCalledWith('google-access-token');

      expect(oauthRepository.findByProvider).toHaveBeenCalledWith('google', 'google-123');
    ...
    it('should login successfully with existing email', async () => {
      ...
      expect(userRepository.findByEmail).toHaveBeenCalledWith('dat@gmail.com');
      ...
      expect(authService.completeLogin).toHaveBeenCalledWith(user, {}, 'google');
    ...
    it('should throw when exchangeCode failed', async () => {
      googleProvider.exchangeCode.mockRejectedValue(new Error('Exchange Code Error'));

      await expect(oauthService.loginWithGoogle('google-code', {})).rejects.toThrow(
        'Exchange Code Error',
      );

      expect(googleProvider.getUserInfo).not.toHaveBeenCalled();
    ...
    it('should throw when getUserInfo failed', async () => {
      ...
      googleProvider.getUserInfo.mockRejectedValue(new Error('Google UserInfo Error'));
    ...
    it('should throw when oauthRepository.create failed for existing email', async () => {
      ...
      oauthRepository.create.mockRejectedValue(new Error('Create OAuth Error'));
    ...
    it('should throw when transaction failed while creating new user', async () => {
      ...
      transactionService.run.mockRejectedValue(new Error('Transaction Error'));
    ...
    it('should throw when completeLogin failed', async () => {
      ...
      authService.completeLogin.mockRejectedValue(new Error('Complete Login Error'));
  ...
  describe('getGoogleAuthorizationUrl', () => {
    it('should generate google authorization url successfully', () => {
      ...
      expect(url.origin + url.pathname).toBe(oauthConfig.google.authorizationUrl);

      expect(url.searchParams.get('client_id')).toBe(oauthConfig.google.clientId);

      expect(url.searchParams.get('redirect_uri')).toBe(oauthConfig.google.redirectUri);

      expect(url.searchParams.get('response_type')).toBe('code');

      expect(url.searchParams.get('scope')).toBe(oauthConfig.google.scopes.join(' '));
