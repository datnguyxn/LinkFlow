...
describe('WorkspaceInvitationService', () => {
  ...
  describe('createInvitation', () => {
    ...
    beforeEach(() => {
      ...
      fixture.workspaceInvitationRepository.findPendingByEmail.mockResolvedValue(null);

      fixture.roleRepository.findById.mockResolvedValue(mockRole);

      fixture.workspaceInvitationRepository.create.mockResolvedValue(mockInvitation);

      fixture.workspaceInvitationPublisher.workspaceInvitationCreated.mockResolvedValue(undefined);
    });

    it('should create invitation successfully', async () => {
      const result = await fixture.workspaceInvitationService.createInvitation(
        workspaceId,
        inviterId,
        input,
        ipAddress,
      ...
      expect(fixture.userRepository.findByEmail).toHaveBeenCalledWith(input.email);
      ...
      expect(fixture.workspaceInvitationRepository.findPendingByEmail).toHaveBeenCalledWith(
        workspaceId,
        input.email,
      );

      expect(fixture.roleRepository.findById).toHaveBeenCalledWith(input.roleId);

      expect(fixture.workspaceInvitationRepository.create).toHaveBeenCalledWith(
      ...
      expect(fixture.workspaceInvitationPublisher.workspaceInvitationCreated).toHaveBeenCalledWith(
        expect.objectContaining({
    ...
    it('should throw NotFoundError when workspace does not exist', async () => {
      ...
      expect(fixture.workspaceInvitationRepository.create).not.toHaveBeenCalled();
    ...
    it('should throw NotFoundError when invitee does not exist', async () => {
      ...
      expect(fixture.workspaceInvitationRepository.findPendingByEmail).not.toHaveBeenCalled();
    ...
    it('should continue when invitee is not an active member', async () => {
      ...
      expect(fixture.workspaceInvitationRepository.create).toHaveBeenCalled();

      expect(fixture.workspaceInvitationPublisher.workspaceInvitationCreated).toHaveBeenCalled();
    });
    ...
    it('should propagate invitation creation errors', async () => {
      fixture.workspaceInvitationRepository.create.mockRejectedValue(new Error('Database error'));
    ...
    it('should propagate publisher errors', async () => {
      ...
      expect(fixture.workspaceInvitationPublisher.workspaceInvitationCreated).toHaveBeenCalledTimes(
        1,
      );
    });
  });
  ...
  describe('listInvitations', () => {
    ...
    it('should list invitations successfully', async () => {
      fixture.workspaceInvitationRepository.findAllByWorkspaceIdWithPagination.mockResolvedValue(
        mockResponse,
      );

      const result = await fixture.workspaceInvitationService.listInvitations(
        workspaceId,
        page,
        limit,
        search,
    ...
    it('should return empty result when no invitations found', async () => {
      const emptyResponse = {
        data: [],
        pagination: {
          page,
      ...
      expect(
        fixture.workspaceInvitationRepository.findAllByWorkspaceIdWithPagination,
      ).toHaveBeenCalledWith(workspaceId, page, limit, undefined);
    });
    ...
    it('should propagate repository errors', async () => {
      fixture.workspaceInvitationRepository.findAllByWorkspaceIdWithPagination.mockRejectedValue(
  ...
  describe('getInvitationById', () => {
    ...
    it('should get invitation successfully', async () => {
      fixture.workspaceInvitationRepository.findById.mockResolvedValue(mockInvitation);

      const result = await fixture.workspaceInvitationService.getInvitationById(invitationId);

      expect(result).toEqual(mockInvitation);

      expect(fixture.workspaceInvitationRepository.findById).toHaveBeenCalledWith(invitationId);
    });
    ...
    it('should throw ConflictError when invitation does not exist', async () => {
      ...
      await expect(
        fixture.workspaceInvitationService.getInvitationById(invitationId),
      ).rejects.toMatchObject({
    ...
    it('should propagate repository errors', async () => {
      fixture.workspaceInvitationRepository.findById.mockRejectedValue(new Error('Database error'));
  ...
  describe('acceptInvitation', () => {
    ...
    it('should reactivate LEFT member successfully', async () => {
  ...
  describe('validateInvitationForAccept', () => {
    ...
    it('should return invitation when token is valid', async () => {
      fixture.workspaceInvitationRepository.findByToken.mockResolvedValue(mockInvitation);

      const result = await (fixture.workspaceInvitationService as any).validateInvitationForAccept(
        token,
      );

      expect(result).toEqual(mockInvitation);

      expect(fixture.workspaceInvitationRepository.findByToken).toHaveBeenCalledWith(token);
    });

    it('should throw NotFoundError when invitation does not exist', async () => {
      fixture.workspaceInvitationRepository.findByToken.mockResolvedValue(null);

      await expect(
        (fixture.workspaceInvitationService as any).validateInvitationForAccept(token),
      ).rejects.toMatchObject({
        code: ERROR_CODE.INVITATION_NOT_FOUND,
      });
    ...
    it('should call handleInvitationExpired and throw GoneError when invitation is expired', async () => {
      ...
      fixture.workspaceInvitationRepository.findByToken.mockResolvedValue(expiredInvitation);
  ...
  describe('handleInvitationExpired', () => {
    ...
    it('should update invitation status to EXPIRED and publish expiration event', async () => {
      ...
      const workspaceInvitationUpdatedSpy = vi
        .spyOn(fixture.workspaceInvitationPublisher, 'workspaceInvitationExpired')
        .mockResolvedValue(undefined);

      await (fixture.workspaceInvitationService as any).handleInvitationExpired(
        expiredEvent,
        ipAddress,
      );
      ...
      expect(workspaceInvitationUpdatedSpy).toHaveBeenCalledWith(
        expect.objectContaining({
    ...
    it('should propagate error when updating invitation status fails', async () => {
      ...
      await expect(
        (fixture.workspaceInvitationService as any).handleInvitationExpired(
          expiredEvent,
          ipAddress,
        ),
      ).rejects.toThrow('Database error');
    ...
    it('should propagate error when publishing expiration event fails', async () => {
      ...
      fixture.workspaceInvitationPublisher.workspaceInvitationExpired = vi
        .fn()
  ...
  describe('revokeInvitation', () => {
    ...
    it('should revoke invitation successfully', async () => {
      ...
      const workspaceInvitationRevokedSpy = vi
        .spyOn(fixture.workspaceInvitationPublisher, 'workspaceInvitationRevoked')
        .mockResolvedValue(undefined);

      const result = await fixture.workspaceInvitationService.revokeInvitation(
        workspaceId,
        invitationId,
        ipAddress,
      );
      ...
      expect(workspaceInvitationRevokedSpy).toHaveBeenCalledWith(
        expect.objectContaining({
          invitationId: mockInvitation.id,
          workspaceId,
          workspaceName: mockWorkspace.name,
    ...
    it('should throw NotFoundError when invitation does not exist', async () => {
      ...
      await expect(
        fixture.workspaceInvitationService.revokeInvitation(workspaceId, invitationId, ipAddress),
      ).rejects.toMatchObject({
        code: ERROR_CODE.INVITATION_NOT_FOUND,
      });
  ...
  describe('rejectInvitation', () => {
    ...
    it('should reject invitation successfully', async () => {
      ...
      const workspaceInvitationRejectedSpy = vi
        .spyOn(fixture.workspaceInvitationPublisher, 'workspaceInvitationRejected')
        .mockResolvedValue(undefined);

      const result = await fixture.workspaceInvitationService.rejectInvitation(token, ipAddress);
      ...
      expect(workspaceInvitationRejectedSpy).toHaveBeenCalledWith(
        expect.objectContaining({
          invitationId: mockInvitation.id,
    ...
    it('should reject invitation successfully with null ip address', async () => {
      ...
      await fixture.workspaceInvitationService.rejectInvitation(token, null);
    ...
    it('should propagate error when invitation validation fails', async () => {
      const error = new GoneError('workspace.invitationExpired', ERROR_CODE.INVITATION_EXPIRED);
      ...
      await expect(
        fixture.workspaceInvitationService.rejectInvitation(token, ipAddress),
      ).rejects.toMatchObject({
        code: ERROR_CODE.INVITATION_EXPIRED,
      });
