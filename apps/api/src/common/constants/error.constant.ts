export const ERROR_CODE = {
  UNAUTHORIZED: 'UNAUTHORIZED',
  FORBIDDEN: 'FORBIDDEN',
  NOT_FOUND: 'NOT_FOUND',
  BAD_REQUEST: 'BAD_REQUEST',
  INTERNAL_SERVER_ERROR: 'INTERNAL_SERVER_ERROR',
  MISSING_REFRESH_TOKEN: 'MISSING_REFRESH_TOKEN',
  INVALID_CREDENTIALS: 'INVALID_CREDENTIALS',
  INVALID_REFRESH_TOKEN: 'INVALID_REFRESH_TOKEN',
  REFRESH_TOKEN_EXPIRED: 'REFRESH_TOKEN_EXPIRED',
  INVALID_TOKEN: 'INVALID_TOKEN',
  USER_INACTIVE: 'USER_INACTIVE',
  USER_SUSPENDED: 'USER_SUSPENDED',
  USER_DELETED: 'USER_DELETED',
  USER_UNAVAILABLE: 'USER_UNAVAILABLE',
  INVALID_EMAIL_VERIFICATION_TOKEN: 'INVALID_EMAIL_VERIFICATION_TOKEN',
  EMAIL_VERIFICATION_TOKEN_EXPIRED: 'EMAIL_VERIFICATION_TOKEN_EXPIRED',
  EMAIL_ALREADY_VERIFIED: 'EMAIL_ALREADY_VERIFIED',
  EMAIL_NOT_VERIFIED: 'EMAIL_NOT_VERIFIED',
  EMAIL_ALREADY_EXISTS: 'EMAIL_ALREADY_EXISTS',
  USER_ALREADY_EXISTS: 'USER_ALREADY_EXISTS',
  INVALID_PASSWORD_RESET_TOKEN: 'INVALID_PASSWORD_RESET_TOKEN',
  PASSWORD_RESET_TOKEN_EXPIRED: 'PASSWORD_RESET_TOKEN_EXPIRED',
  PASSWORD_SAME_AS_OLD: 'PASSWORD_SAME_AS_OLD',
  INVALID_SESSION: 'INVALID_SESSION',

  FILE_REQUIRED: 'FILE_REQUIRED',
  INVALID_FILE_TYPE: 'INVALID_FILE_TYPE',
  INVALID_FILE_EXTENSION: 'INVALID_FILE_EXTENSION',
  EMPTY_FILE: 'EMPTY_FILE',
  FILE_TOO_LARGE: 'FILE_TOO_LARGE',
  FILE_DELETE_FAILED: 'FILE_DELETE_FAILED',

  WORKSPACE_NOT_FOUND: 'WORKSPACE_NOT_FOUND',
  WORKSPACE_ALREADY_EXISTS: 'WORKSPACE_ALREADY_EXISTS',
  WORKSPACE_SLUG_ALREADY_EXISTS: 'WORKSPACE_SLUG_ALREADY_EXISTS',
  WORKSPACE_MEMBER_NOT_FOUND: 'WORKSPACE_MEMBER_NOT_FOUND',
  WORKSPACE_MEMBER_ALREADY_EXISTS: 'WORKSPACE_MEMBER_ALREADY_EXISTS',
  WORKSPACE_MEMBER_ROLE_INVALID: 'WORKSPACE_MEMBER_ROLE_INVALID',
  WORKSPACE_ACCESS_DENIED: 'WORKSPACE_ACCESS_DENIED',
  WORKSPACE_OWNER_ONLY: 'WORKSPACE_OWNER_ONLY',

  INVITATION_ALREADY_EXISTS: 'INVITATION_ALREADY_EXISTS',
  INVITATION_NOT_FOUND: 'INVITATION_NOT_FOUND',
  INVITATION_EXPIRED: 'INVITATION_EXPIRED',
  INVITATION_ALREADY_ACCEPTED: 'INVITATION_ALREADY_ACCEPTED',
  INVITATION_ALREADY_REVOKED: 'INVITATION_ALREADY_REVOKED',
  INVITATION_ALREADY_PROCESSED: 'INVITATION_ALREADY_PROCESSED',

  URL_NOT_FOUND: 'URL_NOT_FOUND',
  URL_ALREADY_EXISTS: 'URL_ALREADY_EXISTS',
  URL_SHORT_CODE_ALREADY_EXISTS: 'URL_SHORT_CODE_ALREADY_EXISTS',
  SHORT_CODE_ALREADY_EXISTS: 'SHORT_CODE_ALREADY_EXISTS',

  ROLE_NOT_FOUND: 'ROLE_NOT_FOUND',
  PERMISSION_NOT_FOUND: 'PERMISSION_NOT_FOUND',
  INVALID_INPUT: 'INVALID_INPUT',

  WORKSPACE_OWNER_REQUIRED: 'WORKSPACE_OWNER_REQUIRED',
  CANNOT_TRANSFER_OWNERSHIP_TO_SELF: 'CANNOT_TRANSFER_OWNERSHIP_TO_SELF',
  WORKSPACE_MEMBER_NOT_ACTIVE: 'WORKSPACE_MEMBER_NOT_ACTIVE',
  MEMBER_ALREADY_HAS_ROLE: 'MEMBER_ALREADY_HAS_ROLE',
  OWNER_ROLE_REQUIRES_TRANSFER: 'OWNER_ROLE_REQUIRES_TRANSFER',
  TRANSFER_OWNERSHIP_BEFORE_LEAVING: 'TRANSFER_OWNERSHIP_BEFORE_LEAVING',
  TRANSFER_OWNERSHIP_BEFORE_REMOVING: 'TRANSFER_OWNERSHIP_BEFORE_REMOVING',
} as const;
