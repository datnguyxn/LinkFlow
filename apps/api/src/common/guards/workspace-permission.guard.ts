...
export function requireWorkspacePermission(permission: WorkspacePermission) {
  return async function (request: FastifyRequest) {
    ...
    const hasPermission = await workspaceRepository.hasPermission(workspaceId, userId, permission);

    if (!hasPermission) {
      throw new ForbiddenError('workspace.permissionDenied', ERROR_CODE.FORBIDDEN);
    }
  };
}
