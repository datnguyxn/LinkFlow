...
export class AdminUserService {
    ...
    async banUser(userId: string, adminId: string, ipAddress: string = "") {
        ...
        const userUpdate = await this.transactionService.run(async (tx) => {
            ...
            const userUpdated = await this.userRepository.update(userId, { status: UserStatus.SUSPENDED }, tx);
    ...
    async changeRole(adminId: string, userId: string, newRole: string, ipAddress: string = "") {
        ...
        const updatedUser = await this.userRepository.update(userId, { role: UserRole[newRole as keyof typeof UserRole] });
    ...
    async restoreUser(userId: string, adminId: string, ipAddress: string = "") {
        ...
        const restoredUser = await this.userRepository.update(userId, { status: UserStatus.ACTIVE, deletedAt: null });
