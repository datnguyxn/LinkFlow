...
export class WorkspaceService {
    ...
    async getWorkspaceById(workspaceId: string, ownerId: string) {
        ...
        const workspace = await this.workspaceRepository.findWorkspaceAndMemberById(workspaceId, ownerId);
     ...
     * @throws ForbiddenError if the user is not a member of the workspace
     */
    async updateWorkspace(workspaceId: string, workspaceData: WorkspaceInput, ownerId: string, ipAddress?: string | null) {
    ...
    private async requireMember(
        ...
        const role =
            await this.workspaceMemberRepository.findRoleByUserId(
                workspaceId,
                userId,
            );
    ...
    private async requireOwner(
        ...
        if (workspace.ownerId !== userId) {
            throw new ForbiddenError(
                'workspace.ownerOnly',
                ERROR_CODE.WORKSPACE_OWNER_ONLY,
            );
        }
