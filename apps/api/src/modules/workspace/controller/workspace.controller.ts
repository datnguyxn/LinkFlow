...
export class WorkspaceController {
    ...
    async createWorkspace(
        ...
        if (!newWorkspace) {
            return ResponseHandler.error(reply, HTTP_STATUS.INTERNAL_SERVER_ERROR, 'workspace.creationFailed');
        }
    ...
    async getAllWorkspaces(
        ...
        if (!workspaces) {
            return ResponseHandler.error(reply, HTTP_STATUS.INTERNAL_SERVER_ERROR, 'workspace.retrievalFailed');
        }
    ...
    async updateWorkspace(
        ...
        const updatedWorkspace = await this.workspaceService.updateWorkspace(workspaceId, data, id, ipAddress);

        // If the workspace update fails, return an error response indicating that the update failed
        if (!updatedWorkspace) {
            return ResponseHandler.error(reply, HTTP_STATUS.INTERNAL_SERVER_ERROR, 'workspace.updateFailed');
        }
    ...
    async deleteWorkspace(
        ...
        const deletedWorkspace = await this.workspaceService.deleteWorkspace(workspaceId, id, ipAddress);

        // If the workspace deletion fails, return an error response indicating that the deletion failed
        if (!deletedWorkspace) {
            return ResponseHandler.error(reply, HTTP_STATUS.INTERNAL_SERVER_ERROR, 'workspace.deletionFailed');
        }
    ...
    async restoreWorkspace(
        ...
        const restoredWorkspace = await this.workspaceService.restoreWorkspace(workspaceId, id, ipAddress);

        // If the workspace restoration fails, return an error response indicating that the restoration failed
        if (!restoredWorkspace) {
            return ResponseHandler.error(reply, HTTP_STATUS.INTERNAL_SERVER_ERROR, 'workspace.restoreFailed');
        }
