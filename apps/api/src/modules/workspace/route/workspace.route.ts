...
import { type CreateWorkspaceInvitationInput, createWorkspaceInvitationSchema } from '../validator/workspace-invitation.validator.ts';
...
export const workspaceRoutes = async (app: FastifyInstance) => {
    ...
    app.post<{ Body: CreateWorkspaceInvitationInput, Params: { id: string } }>(
        '/:id/invitations',
        {
            ...
            preHandler: [requireWorkspacePermission(
                WORKSPACE_PERMISSION.INVITATION_CREATE,
            ),], // Ensure user is authenticated before processing the request
    ...
    app.delete<{ Params: { id: string, invitationId: string } }>(
        '/:id/invitations/:invitationId',
        {
            ...
            preHandler: [requireWorkspacePermission(
                WORKSPACE_PERMISSION.INVITATION_CANCEL,
            ),], // Ensure user is authenticated before processing the request
        },
