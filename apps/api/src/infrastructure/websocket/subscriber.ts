import { redisSubscriber } from '../cache/subscriber.ts';
...
export async function startWebSocketSubscriber() {
  await redisSubscriber.subscribe(REDIS_CHANNEL.NOTIFICATION_CREATED, (message) => {
  ...
  console.log('✅ WebSocket Redis subscriber started');
}
