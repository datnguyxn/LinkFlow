...
export class WorkspaceInvitationExpirationJob {
  ...
  constructor(
    ...
    private readonly workspaceInvitationPublisher: WorkspaceInvitationPublisher,
  ) {}

  async start() {
    console.log('⏰ Workspace invitation expiration job started');
  ...
  async stop() {
    ...
    console.log('🛑 Workspace invitation expiration job stopped');
  }

  private async scheduleNext() {
    const nextInvitation = await this.workspaceInvitationRepository.findNextPendingExpiration();

    if (!nextInvitation) {
      return;
    }

    const delay = Math.max(nextInvitation.expiresAt.getTime() - Date.now(), 0);
  ...
  private async run() {
    try {
      const invitations = await this.workspaceInvitationRepository.findExpiredPendingInvitations();

      for (const invitation of invitations) {
        const updatedInvitation = await this.workspaceInvitationRepository.expire(invitation.id);
      ...
      if (invitations.length > 0) {
        console.log(`⏰ Expired ${invitations.length} invitation(s)`);
      }

      await this.scheduleNext();
    } catch (error) {
      console.error('❌ Workspace invitation expiration job failed:', error);
